...
class HCTnet(nn.Module):
    ...
    def __init__(
        ...
        self.conv2d_features = nn.Sequential(
            nn.Conv2d(in_channels=8*(c1-2), out_channels=64, kernel_size=(3, 3)),
            nn.BatchNorm2d(64),
            nn.ReLU(),
        )
        self.conv2d_features2 = nn.Sequential(
            nn.Conv2d(c2, out_channels=64, kernel_size=(3, 3)),
            nn.BatchNorm2d(64),
            nn.ReLU(),
        )
        # Tokenization
    ...
    def forward(self, x1, x2, mask = None):
        ...
        # x1 B C (pp) ->B 1 C p p
        _,_,pp=x1.shape
        x1 = rearrange(x1, 'b c (h w) -> b 1 c h w ',h=int(pp**0.5))
...
if __name__ == '__main__':
    model = HCTnet(100,1,12)
    model.eval()
    print(model)
    # input1 = torch.randn(64, 1, 100, 11, 11)
    # input2 = torch.randn(64, 1, 11, 11)
    # x = model(input1, input2)
    # print(x.size())
    summary(model, torch.zeros((64, 100, 121)), torch.zeros((64, 1, 121)))
